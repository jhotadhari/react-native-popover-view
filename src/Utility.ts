...
export function getRectForRef<T extends View>(ref: RefObject<T>): Promise<Rect> {
...
export async function waitForNewRect<T extends View>(
  ref: RefObject<T>,
  initialRect: Rect
): Promise<Rect> {
  await waitForChange(
...
export function getBorderRadius(popoverStyle: StyleProp<ViewStyle>): number {
  const value = StyleSheet.flatten(popoverStyle).borderRadius?.valueOf();
